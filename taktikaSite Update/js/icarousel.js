...
(function($) {
	...
	var iCarousel = function(el, slides, options) {
		...
		//Fix slides number
		options.slides = (options.slides > ic.defs.total) ? defs.total : options.slides;
		...
		if (options.autoPlay && ic.defs.total > 1 && ic.defs.timer != "bar") {
			//Start the Raphael
			...
			if (options.autoPlay && ic.defs.total > 1 && ic.defs.timer == "pie") {
		...
		if (options.autoPlay && ic.defs.total > 1 && ic.defs.timer == "360bar") {
			ic.timerBgPath.attr({
				arc: [359.9, R]
			});
		}
		...
		// Run Preloader
		new ImagePreload(ic.defs.images, function(i) {
			var percent = (i * 10);
	...
	iCarousel.prototype = {
		...
		init: function() {
			...
			if (ic.options.autoPlay && ic.defs.total > 1) {
				ic.setTimer();
				ic.iCarouselTimer.attr('title', ic.options.pauseLabel).show();
			}
		},
		...
		animationEnd: function(ic) {
			...
			if (ic.defs.interval == null && !ic.defs.pause && ic.options.autoPlay) ic.setTimer();
		},
		...
		layout: function(animate, speedTime) {
			...
			for (var i = 0; i < ic.rightItems.length; i++) {
				...
				zIndex -= i + 1, css = this.CSS(slide, i, zIndex, true), cssA = ic.CSS(slide, i, zIndex, true, true);
			...
			if (ic.defs.total > ic.options.slides) {
				ic.rightOutItem.css(ic.CSS(ic.rightOutItem, ic.leftItems.length - 0.5, ic.leftItems.length - 1, true));
				ic.leftOutItem.css(ic.CSS(ic.leftOutItem, ic.leftItems.length - 0.5, ic.leftItems.length - 1));
			}
		...
		CSS: function(slide, i, zIndex, positive) {
			...
			if (ic.support.transform3d() && ic.options.make3D) {
				var transform = (positive) ? 'translateX(' + (leftRemain) + 'px) translateZ(-' + (250 + ((i + 1) * 110)) + 'px) rotateY(-' + ic.options.perspective + 'deg)' : 'translateX(-' + (leftRemain) + 'px) translateZ(-' + (250 + ((i + 1) * 110)) + 'px) rotateY(' + ic.options.perspective + 'deg)',
					...
					height = "none",
					overflow = "visible";
			} else if (ic.support.transform2d()) {
		...
		events: function() {
			...
			if (ic.options.pauseOnHover) {
				ic.el.hover(function() {
					...
					//Restart the timer
					if (!ic.defs.lock && !ic.defs.pause && ic.defs.interval == null && ic.defs.degree <= 359 && ic.options.autoPlay) {
						ic.setTimer();
					}
				});
			}
			...
			//Touch navigation
			if (ic.options.touchNav && (navigator.userAgent.match(/ipad|iphone|ipod|android/i))) {
				ic.el.swipe({
					swipeLeft: function() {
